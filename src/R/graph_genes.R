...
graph_gene <- function(df,gene.data,intron=FALSE) {
  ...
  df <- subset(df,chr==chrom & cat=="mRNA" & start >= gene.start & stop <= gene.stop)
...
gg_wt <- wt %.% filter(count>9,cat=="mRNA") %.% mutate(sample="WT_DMSO_Rep1")
graph_gene(gg_wt,SPT6)

# Genes I've graphed
SPT6 = c("chr7",720409,724764,"SPT6","+")
